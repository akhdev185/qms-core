  ...
  React.useEffect(() => {
    ...
    const { data: { subscription } } = supabase.auth.onAuthStateChange(async (event, session) => {
      ...
      if ((event === 'SIGNED_IN' || event === 'TOKEN_REFRESHED') && session?.user) {
        ...
        try {
          console.log("[TRACE] listener: fetching profile...");
          const [profRes, roleRes] = await withTimeout<any>(Promise.all([
          ...
          console.log("[TRACE] listener: profile & role resolved.");
          profile = profRes.data as ProfileRow;
          roleData = roleRes.data as { role: string };

          if (!profile) {
            console.log("[AUTH] Profile missing for existing session user.");
            // We do NOT create a new profile here anymore as we removed Google Sign-Up.
            // Only manual registration or admin creation adds users.
          }
        } catch (err) {
          console.error("[AUTH] Profile fetch stalled or failed in listener:", err);
          // Fallback to minimal user if we have a session but fetch failed
        ...
        if (profile) {
          ...
          setUser(appUser);
          saveSession(authUserId, appRole); // IMPORTANT: Cache role here too
          setUsers(prev => {
            if (prev.some(u => u.id === appUser.id)) return prev;
            return [appUser, ...prev];
          });
